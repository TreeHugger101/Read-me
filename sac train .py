...
W_MAX = 20.0
PQR_MAX = 2.0
ALT_ERR_MAX = 5000.0   # better scaling
...
class UAVEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # =====================================================
        # REWARD (stable UAV control)
        # =====================================================

        alt_err = z - Z_TARGET

        # Main altitude hold (quadratic cost)
        reward = -0.01 * (alt_err ** 2)

        # Smoothness penalties
        reward -= 0.05 * abs(q)
        reward -= 0.02 * abs(v)
        reward -= 0.1 * (roll**2 + pitch**2)
...
if __name__ == "__main__":
    ...
    if os.path.exists(MODEL_PATH + ".zip"):
        ...
        try:
            ...
            print("Old model incompatible, creating new one...")
